...
class Model(BaseModel):
    ...
    def name_validator(cls, name: str) -> str:
        ...
        if (
            ...
            and config.client_casing == ClientCasing.lower_case
            and iskeyword(name.lower())
        ):
            raise ValueError(
    ...
    def instance_name(self) -> str:
        ...
        if (
            isinstance(config, Config)
            and config.client_casing == ClientCasing.camel_case
        ):
            return to_camel_case(self.name)
        elif (
            isinstance(config, Config)
            and config.client_casing == ClientCasing.pascal_case
        ):
            return to_pascal_case(self.name)
        elif (
            isinstance(config, Config)
            and config.client_casing == ClientCasing.snake_case
        ):
            return to_snake_case(self.name)
        elif (
            isinstance(config, Config)
            and config.client_casing == ClientCasing.upper_case
        ):
