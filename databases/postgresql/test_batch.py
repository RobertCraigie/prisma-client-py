...
async def test_create_many(client: Prisma) -> None:
    """create_many action works as suggested"""
    async with client.batch_() as batcher:
        batcher.user.create_many([{'name': 'Robert'}, {'name': 'Tegan'}])
        batcher.user.create_many([{'name': 'Alfie'}])
        assert await client.user.count() == 0
    ...
    assert len(users) == 3
    assert users[0].name == 'Alfie'
    assert users[1].name == 'Robert'
    assert users[2].name == 'Tegan'
