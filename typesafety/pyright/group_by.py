...
async def order() -> None:
    ...
    await Profile.prisma().group_by(
        ['city'],
        order={  # E: Argument of type "dict[str, str]" cannot be assigned to parameter "order" of type "Mapping[ProfileScalarFieldKeysT@group_by, SortOrder] | None" in function "group_by"
            'country': 'desc',
        },
    )
